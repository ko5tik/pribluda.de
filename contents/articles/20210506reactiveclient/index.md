...
Here comes why:
- Gateway app manager user and authority entities and is completely generated by JHipster, and I like to keep it. Or maybe switch from database to LDAP later
...
Just derive from generated intoface and add ome annotatons:
```java
import reactivefeign.spring.config.ReactiveFeignClient;
import de.vc.admin.client.recorder.api.UserProfileApi;

@ReactiveFeignClient(name = "${default.name:default}", url = "${default.url:http://localhost:8081/api}")
...
we need to annotate our main app  class with **@EnableReactiveFeignClients**

```java
//  important import!!!! ( others are omitted for brevity)
import reactivefeign.spring.config.EnableReactiveFeignClients;


//  thi one is important
@EnableReactiveFeignClients
// this one too
@EnableFeignClients
@ComponentScan(excludeFilters = { @ComponentScan.Filter(type = FilterType.REGEX, pattern = "de.xxxxx.*.ClientConfiguration") })
...
// this class ois generated by jhipster  and gives us  bearer JWT token associated with current invocation  
import de.xxxxxxx.SecurityUtils;

// those are important,  our feign client implementation looks for them them
...
public class UserFeignClientInterceptor implements ReactiveHttpRequestInterceptor {
    ...
    //  here we manipulate request and add bearer token to it
    public Mono<ReactiveHttpRequest> apply(ReactiveHttpRequest reactiveHttpRequest) {
        // everything is async and reactive, so security util gives us just a mono of token 
        return SecurityUtils.getCurrentUserJWT()
                //  at some later time, this lambda will be executed 
                .map(jwt ->
        {
            //  we add header to request
            reactiveHttpRequest.headers().put(AUTHORIZATION_HEADER, List.of(String.format("%s %s", BEARER, jwt)));
            //  and map function will wrap it into proper mono so  reactive magic can happen
